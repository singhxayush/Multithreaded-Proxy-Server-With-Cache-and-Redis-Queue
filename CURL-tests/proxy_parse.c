...
void send_response(int client_socket, struct HttpResponse *cache)
{
     ...
     sprintf(buffer, "Server: Local-Proxy-Server/1.0\r\n");
     send(client_socket, buffer, strlen(buffer), 0);
     ...
     while (offset < cache->body_len)
     {
          size_t remaining = cache->body_len - offset;
          size_t current_chunk_size = remaining < chunk_size ? remaining : chunk_size;
          send(client_socket, cache->body + offset, current_chunk_size, 0);
...
void fetch_url(const char *url, struct HttpResponse *cache)
{
     ...
     struct MemoryStruct headers_chunk = {malloc(1), 0};

     if (chunk.memory == NULL || headers_chunk.memory == NULL)
     {
          fprintf(stderr, "Failed to allocate memory for fetch_url\n");
          if (chunk.memory)
               free(chunk.memory);
          if (headers_chunk.memory)
...
struct HttpResponse *initialize_empty_http_response()
{
    ...
    // Initialize headers and body to NULL and lengths to 0
    res->headers = NULL;
    res->headers_len = 0;
    res->body = NULL;
    res->body_len = 0;
