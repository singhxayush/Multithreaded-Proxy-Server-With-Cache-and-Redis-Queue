/*
 * proxy_parse.h -- a HTTP/1.x Request Parser/Handler(modification) Library.
 * Written by: Matvey Arye
 * Modified by: Ayush Kumar
 */
...
/*
   This is used to store the parsed incoming request from the client.

   ParsedRequest objects are created from parsing a buffer containing a HTTP request.

   The request buffer consists of a request line followed by a number of headers.

   Request line fields such as method, protocol etc. are stored explicitly.

   Headers are stored in a linked list. Each node in this list is a "ParsedHeader" and contains a key-value pair.
   - char *method;
   - char *protocol;
   - char *host;
   - char *port;
   ...
   The buf and buflen fields are used internally to maintain the parsed request line.
*/
...
/*
   - ParsedHeader: any header after the request line is a key-value pair

   - Format "key:value\r\n"

   - It is maintained in the ParsedHeader linked list within ParsedRequest
*/
struct ParsedHeader
{
   char *key;
...
/*
   - This function creates an empty parsing object

   - Returns a zero-initialized struct variable - ParsedRequest

   - Which is then to be used exactly once for parsing a single request buffer
*/
...
void send_response(int client_socket, struct HttpResponse *cache);
void fetch_url(const char *url, struct HttpResponse *cache);
struct HttpResponse *initialize_empty_http_response();
